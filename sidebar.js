...
var ITEMS_NAV = {
  ...
  chat:        { id: 'chat',        label: 'Chat',           url: BASE+'chat.html',      icon: '<path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/>', badge: true, badgeId: 'chatNavBadge' },
...
var NAV_POR_ROL = {
  ...
  agente: {
    principal: ['home', 'bandeja', 'actce', 'chat', 'flash', 'notificaciones', 'solicitudes', 'biblioteca', 'miturno'],
    reportes:  ['metricas', 'transporte', 'perfil']
  },
  'agente-inmersion': {
    principal: ['home', 'bandeja', 'actce', 'chat', 'flash', 'notificaciones', 'biblioteca', 'miturno'],
  ...
  supervisor: {
    principal: ['sup-jornada','sup-turno','sup-monitoreo','sup-bases','sup-actce','sup-chat','sup-metricas','sup-flash','sup-notif','sup-solicitudes','sup-acciones','sup-licencias','sup-badges','sup-biblioteca','sup-transporte','sup-perfil'],
    reportes:  []
  },
  wfm: {
    principal: ['wfm-jornada','jefe-usuarios','jefe-solicitudes','jefe-tiemporeal','jefe-metricas','jefe-turno','jefe-reportes','jefe-transporte','jefe-perfil'],
    reportes:  []
  },
  superadmin: {
...
function cargarChatBadge(uid, db) {
  db.collection('chats').where('participantes', 'array-contains', uid).get()
    .then(function(snap) {
      ...
      snap.forEach(function(d) {
        var c = d.data();
        if (!c.ultimoMensaje || c.ultimoMensaje.autorUid === uid) return;
        var leido = c.leidoHasta && c.leidoHasta[uid];
